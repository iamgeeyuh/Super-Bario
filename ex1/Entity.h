...
enum AnimationDirection { IDLE, MOVE, HURT, JUMP, ATTACK };
...
private:
    ...
    int m_walking[5][4]; // 4x4 array for walking animations
...
public:
    ...
    // ————— METHODS ————— //
    Entity();
    Entity(GLuint texture_id, float speed, glm::vec3 acceleration, float jump_power, int walking[5][4], float animation_time,
    ...
    void ai_combee(Entity *player);
    
    void normalise_movement() { m_movement = glm::normalize(m_movement); }
    
        void hurt(float enemy_x) {
//            m_is_alive = false;
...
//            m_play_death_sound = true;
            ...
            m_animation_indices = m_walking[HURT];
    ...
    void move() { if (!m_jumping_animation && hurt_timer <= 0.0f) m_animation_indices = m_walking[MOVE]; }
    void idle() { if (!m_jumping_animation && hurt_timer <= 0.0f) m_animation_indices = m_walking[IDLE]; }
    ...
    bool      const get_play_hurt_sound() const { return play_hurt_sound; }
    ...
    void const set_play_hurt_sound(bool new_play_hurt_sound) { play_hurt_sound = new_play_hurt_sound; }
    
    // Setter for m_walking
    void set_walking(int walking[5][4])
    {
        for (int i = 0; i < 5; ++i)
        {
            for (int j = 0; j < 4; ++j)
